...
import { WalletIcon, MailIcon, LockIcon, ArrowRightIcon, CalculatorIcon, UserIcon } from 'lucide-react';
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        ...
        if (loginRes?.error) throw new Error('Login otomatis gagal, silakan login manual');
        router.push('/');
      } else {
    ...
    <main className="min-h-screen bg-background text-foreground flex items-center justify-center p-4 selection:bg-accent/30 selection:text-accent relative overflow-hidden transition-colors duration-300">
      ...
      <div className="w-full max-w-md animate-fade-in relative z-10">
        <div className="text-center mb-12">
          <div className="inline-flex p-4 bg-accent/10 rounded-[2rem] mb-6 ring-1 ring-accent/20">
          ...
          <p className="text-gray-500 text-lg">
            {isRegister ? 'Join Illyas Finance' : 'Sign in to your account'}
          </p>
        </div>
        ...
        <div className="glass-card p-10 rounded-[2.5rem] shadow-2xl">
          ...
          <div className="mt-10 pt-8 border-t border-white/5 text-center space-y-4">
            ...
            >
              {isRegister ? 'Already have an account? Sign in' : "Don't have an account? Create one now"}
            </button>
            <Link
              href="/financial-freedom"
              className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-accent/5 hover:bg-accent/10 text-accent transition-all text-sm font-bold border border-accent/10"
            >
